...
{
  public partial class frmRecentDestinationList : frmCarApplicationBase
  {
    public frmRecentDestinationList()
    {
      InitializeComponent();
    ...
    protected RecentDestinationDataset.RecentDestinationDataTable DataSource
    {
      get { return (RecentDestinationDataset.RecentDestinationDataTable)pageController.DataSource; }
      set
      {
        if (value == null)
          return;
    ...
    protected RecentDestinationDataset.RecentDestinationRow SelectedRow
    {
      get
      {
        ...
        return ((RecentDestinationDataset.RecentDestinationRow)(((DataRowView)(pageController.PageData[SelectedIndex])).Row));
      }
    }
    ...
    private void RowMoreClicked()
    {
      CarApplication.Instance.WriteLog(this, ">> RowMoreClicked");

      if (SelectedRow == null)
      {
        CarApplication.Instance.WriteLog(this, "   SelectedRow == null");
        return;
      }
      if (SelectedRow.IsDestinationIdNull() == true)
      {
        CarApplication.Instance.WriteLog(this, "   SelectedRow.IsDestinationIdNull() == true");
        return;
      }
      ...
      try
      {
        Destination = CarApplication.Instance.DestinationBll.GetById(SelectedRow.DestinationId);
      }
      catch(Exception exc)
      {
        CarApplication.Instance.WriteLog(this,"Error",exc);
      ...
      if (Destination == null)
      {
        CarApplication.Instance.WriteLog(this, "   Destination == null");
        return;
      }

      MoreResults Result = frmDestinationMore.ShowMore(Destination,true);
      if ((Result == MoreResults.TakeMeThere) || (Result == MoreResults.ViewMap))
        Close();
    }
    private RecentDestinationDataset.RecentDestinationDataTable GetData()
    {
      RecentDestinationDataset.RecentDestinationDataTable Destinations;
      try
      {
        Destinations = CarApplication.Instance.RecentDestinationBll.GetAll();
      }
      catch (Exception exc)
      {
        return null;
      ...
      if (Position != null)
      {
        CarApplication.Instance.RecentDestinationBll.CalculateDistanceDirection(Destinations, Position.Latitude, Position.Longitude,CarApplication.Instance.Units);
      }
    ...
    private void frmRecentDestinationList_Load(object sender, EventArgs e)
    {
      DataSource = GetData();
    }
